...
/**
 * This class defines the Apache Beam pipeline that was used to benchmark the watermark subsystems of
 * Google Cloud Dataflow and Apache Flink for the paper
 * "Watermarks in Stream Processing Systems: Semantics and Comparative Analysis of Apache Flink and
 * Google Cloud Dataflow", submitted for the Industrial Track of VLDB 2021 by Tyler Akidau, Edmon Begoli,
 * Slava Chernyak, Fabian Hueske, Kathryn Knight, Kenneth Knowles, Daniel Mills, and Dan Sotolongo.
 *
 */
                          ...
                          new DoFn<KV<Long, Iterable<Instant>>, KV<Long, Instant>>() {
                            ...
                            public void process(ProcessContext c, @Timestamp Instant ts) {
                              ...
                              if (idx > 0) {
                                ...
                                c.output(latencyResult, KV.of(String.format("GBK%d-GBK%d", idx - 1, idx), sessionDelay));
                              }
                            }
                          })
                          ...
                          new DoFn<KV<String, Iterable<Duration>>, String>() {
                            ...
                            public void process(ProcessContext c) {
                              ...
                              String out = String.format("%s, %s, %d, %d, %d, %d, %d",
                                      configName, c.element().getKey(),
                                      median.getMillis(), p75.getMillis(), p95.getMillis(), p99.getMillis(), numElements);
                              LOG.info(out);
                            }
                            ...
                            private void computePercentiles(Iterable<Duration> vals) {
                              ...
                              if (medianIndex < 0 || medianIndex >= accumulator.size() ||
                                      p75Index < 0 || p75Index >= accumulator.size() ||
                                      p95Index < 0 || p95Index >= accumulator.size() ||
                                      p99Index < 0 || p99Index >= accumulator.size()) {
